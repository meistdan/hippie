...
//          while (nodeAddr >= 0 && nodeAddr != EntrypointSentinel)
            while ((unsigned int)(nodeAddr) < (unsigned int)(EntrypointSentinel))   // functionally equivalent, but faster
            {
                ...
                const float4 n0xy = nodes[4 * nodeAddr + 0]; // (c0.lo.x, c0.hi.x, c0.lo.y, c0.hi.y)
                const float4 n1xy = nodes[4 * nodeAddr + 1]; // (c1.lo.x, c1.hi.x, c1.lo.y, c1.hi.y)
                const float4 nz = nodes[4 * nodeAddr + 2]; // (c0.lo.z, c0.hi.z, c1.lo.z, c1.hi.z)
                float4 tmp = nodes[4 * nodeAddr + 3]; // child_index0, child_index1
...
//              if (nodeAddr < 0 && leafAddr2 >= 0)     // Postpone max 2
            ...
            {
                // Triangle bounds.
                const float4 triBounds = nodes[4 * (~leafAddr) + 3];
                for (int triAddr = __float_as_int(triBounds.x); triAddr < __float_as_int(triBounds.y); ++triAddr)
                {
...
do
{
    ...
    if (terminated)
    {
        ...
        int rayidxTmp = indices[rayidx];
        float4 o = FETCH_GLOBAL(rays, rayidxTmp * 2 + 0, float4);
        float4 d = FETCH_GLOBAL(rays, rayidxTmp * 2 + 1, float4);
        origx = o.x;
        origy = o.y;
        origz = o.z;
        tmin = o.w;
