            ...
            for rr in russian_roulette:
                for l in ray_length:
                    for s in scene_index:
                        scene = scenes[s]
                        speedups_overall = np.zeros(len(hilbert_codes) * len(morton_code_method))
                        labels = []
                        for mi in range(len(morton_code_method)):
                            for hi in range(len(hilbert_codes)):
                                mm = morton_code_method[mi]
                                hc = hilbert_codes[hi]
                                label = ("hc-" if hc else "mc-") + mm
                                labels.append(label)
                                for c in range(number_of_cameras):
                                    test_name = scene + "-cam=" + str(c) + "-rd=" + str(r) + "-spp=" + str(spp) + "-len=" + ('%.3f' % l)\
                                        + "-mb=" + str(mb) + "-df=" + str(int(dynamic_fetch)) + "-hc="  + \
                                    ...
                                    speedups_overall[len(hilbert_codes) * mi + hi] += sum(speedups) / (r * number_of_cameras)
                        file_name = scene + "-rd=" + str(r) + "-spp=" + str(spp) + "-len=" + (
                            '%.3f' % l) + "-rr=" + str(int(rr)) + "-mb=" + str(mb) + "-df=" + str(int(dynamic_fetch)) + "-" + str(height) + "p"
                        plt.figure()
                        plt.bar(range(len(hilbert_codes) * len(morton_code_method)), speedups_overall)
                        plt.xticks(range(len(hilbert_codes) * len(morton_code_method)), labels)
