                ...
                for rr in russian_roulette:
                    for l in ray_length:
                        for s in scene_index:
                            scene = scenes[s]
                            speedups_overall = np.zeros(len(morton_code_bits))
                            labels = []
                            for mi in range(len(morton_code_bits)):
                                mb = morton_code_bits[mi]
                            ...
                            '%.3f' % l) + "-rr=" + str(int(rr)) + "-hc=" + str(int(hc)) + "-mm=" + str(mm) + "-" + str(
                                height) + "p"
                            plt.figure()
                            plt.bar(range(len(morton_code_bits)), speedups_overall)
                            plt.xticks(range(len(morton_code_bits)), labels)
